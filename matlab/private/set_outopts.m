...
% rev. 3 : (04-19-2016) added VideoFilters and AudioFilters options
...
for n = 1:numel(fnames)
   ...
   switch fnames{n}
      ...
      case 'VideoFilters'
         [fg,iscomplex] = ffmpegfiltergraph(val);
         if ~iscomplex
            outopts(1).vf = sprintf('"%s"',fg);
         end
      case 'AutioFilters'
         [fg,iscomplex] = ffmpegfiltergraph(val);
         if ~iscomplex
            outopts(1).af = sprintf('"%s"',fg);
         end
      case 'OutputCustomOptions'
         cfnames = fieldnames(val);
         for k = 1:numel(cfnames)
            if iscell(val.(cfnames{k}))
               optval = '';
               for m = 2:numel(val.(cfnames{k}))
                  optval = sprintf('%s -%s %s',optval,cfnames{k},val.(cfnames{k}){m});
               end
               outopts(1).(cfnames{k}) = sprintf('%s%s',val.(cfnames{k}){1},optval);
            else
