...
%      VideoFilters     [Array of ffmpegfilters]
%                       A filtergraph to filter the video output
%      AudioFilters     [Array of ffmpegfilters]
%                       A filtergraph to filter the audio output
...
% rev. 1 : (04-19-2016)
%          Added VideoFilters and AudioFilters options
...
args = {
   ...
   'VideoFilters',    [],     @(v)isa(v,'ffmpegfilter.base')
   'AudioFilters',    [],     @(v)isa(v,'ffmpegfilter.base')
   'OutputCustomOptions', [], @(val)validateattributes(val,{'struct'},{'scalar'})
   };
...
if ischar(val)
   ...
   validateattributes(val,{'numeric'},{'scalar','nonnegative','finite'});
end
tf = true;
end
