function addOutputParameters(p,defvals,altlist,optacodecs,optvcodecs)
   ...
   % rev. 1 : (09-09-2019) fixed codec name checks
   ...
   args = {
      'AudioCodec',   'copy',    @(val)checkcodec(val,optacodecs,1)
      ...
      'VideoCodec',   'copy',    @(val)checkcodec(val,optvcodecs,0)
   ...
   function tf = checkcodec(val,opts,audio)
   
   tf = any(strcmpi(val,opts));
   if (~tf)
      tf = iscodec(val,audio,true);
   end
   end
