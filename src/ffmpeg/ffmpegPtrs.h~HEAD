...
inline void delete_dict(AVDictionary *dict) { av_dict_free(&dict); }

typedef std::unique_ptr<AVCodecContext, std::function<void(AVCodecContext *)>> CodecCtxPtr;
inline void delete_codec_ctx(AVCodecContext *ctx) { avcodec_free_context(&ctx); }

typedef std::unique_ptr<AVFormatContext, std::function<void(AVFormatContext *)>> FormatCtxPtr;
inline void delete_input_ctx(AVFormatContext *ctx) { avformat_close_input(&ctx); }
