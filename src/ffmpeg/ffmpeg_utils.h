...
#define AVDictionaryAutoDelete(dict)                                           \
  std::unique_ptr<AVDictionary, decltype(&av_dict_delete)> cleanup_##dict(     \
      dict, &av_dict_delete)
...
#define AVCodecContextAutoDelete(ctx)                                          \
  std::unique_ptr<AVCodecContext, decltype(&av_codec_context_delete)>          \
      cleanup_##ctx(ctx, &av_codec_context_delete)
#define AVCodecContextUniquePtr                                                \
  std::unique_ptr<AVCodecContext, decltype(&av_codec_context_delete)>
#define NewAVCodecContextUniquePtr(ctx)                                        \
  std::unique_ptr<AVCodecContext, decltype(&av_codec_context_delete)>(         \
      ctx, &av_codec_context_delete)
