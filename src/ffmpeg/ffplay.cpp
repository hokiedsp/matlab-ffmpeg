...
int decoder_decode(AVCodecContext *avctx, AVFrame *frame, int *got_frame, AVPacket *pkt)
{
   int ret;
...
int decoder_decode_frame(Decoder *d, AVFrame *frame, AVSubtitle *sub)
{
   ...
   do
   {
      ...
      ret = decoder_decode(d->avctx, frame, &got_frame, &d->pkt_temp);
...
Frame *frame_queue_peek_writable(FrameQueue *f)
{
   ...
   {
      std::unique_lock<std::mutex> lk(f->mutex);
      f->cond.wait(lk);
   }
   f->mutex.unlock();
...
int realloc_texture(Texture **texture, uint32_t new_format, int new_width, int new_height, BlendMode blendmode, int init_texture)
{
   uint32_t format;
   int access, w, h;
   ...
   //    if (!(*texture = SDL_CreateTexture(renderer, new_format, TextureACCESS_STREAMING, new_width, new_height)))
   //       return -1;
...
int upload_texture(Texture *tex, AVFrame *frame, SwsContext **img_convert_ctx)
{
   int ret = 0;
   switch (frame->format)
   {
            ...
            if (vp->pts >= sp->pts + ((float)sp->sub.start_display_time / 1000))
            {
               if (!sp->uploaded)
               {
                  ...
                  for (i = 0; i < (int)sp->sub.num_rects; i++)
                  {
                     ...
                     // if (!SDL_LockTexture(is->sub_texture, (Rect *)sub_rect, (void **)pixels, pitch))
                     // {
...
// #else
         ...
         for (i = 0; i < (int)sp->sub.num_rects; i++)
         {
            Rect *sub_rect = (Rect *)sp->sub.rects[i];
            Rect target = {rect.x + (int)(sub_rect->x * xratio), rect.y + (int)(sub_rect->y * yratio), (int)(sub_rect->w * xratio), (int)(sub_rect->h * yratio)};
...
int queue_picture(VideoState *is, AVFrame *src_frame, double pts, double duration, int64_t pos, int serial)
{
       ...
       vp->format != src_frame->format)
   {
      ...
      vp->width = src_frame->width;
      vp->height = src_frame->height;
      vp->format = (AVPixelFormat)src_frame->format;
      ...
      std::unique_lock<std::mutex> lk(is->pictq.mutex);
...
int decoder_start(Decoder *d, int (*fn)(void *), void *arg)
{
   ...
   d->decoder_tid = std::thread(fn,arg); //SDL_CreateThread(fn, "decoder", arg);
   if (!d->decoder_tid.joinable())
   {
      av_log(NULL, AV_LOG_ERROR, "thread failed to start\n");
      return AVERROR(ENOMEM);
   }
   return 0;
}
...
int read_thread(void *arg)
{
   ...
   for (;;)
   {
                                                                                       ...
                                                                                       stream_has_enough_packets(is->subtitle_st, is->subtitle_stream, &is->subtitleq))))
      {
         ...
         is->continue_read_thread.wait_for(lk, 10ms);
         continue;
      }
      if (!is->paused &&
...
VideoState *stream_open(const char *filename, AVInputFormat *iformat)
{
   ...
   {
      av_log(NULL, AV_LOG_FATAL, "Failed to start read_thread() thread\n");
   fail:
      stream_close(is);
      return NULL;
   }
...
{
   show_mode = !strcmp(arg, "video") ? SHOW_MODE_VIDEO : !strcmp(arg, "waves") ? SHOW_MODE_WAVES : !strcmp(arg, "rdft") ? SHOW_MODE_RDFT : (ShowMode)(int)parse_number_or_die(opt, arg, OPT_INT, 0, SHOW_MODE_NB - 1);
   return 0;
}
...
#include "cmdutils_common_opts.h"
    {"x", HAS_ARG, OptionAction(opt_width), "force displayed width", "width"},
    {"y", HAS_ARG, OptionAction(opt_height), "force displayed height", "height"},
    {"s", HAS_ARG | OPT_VIDEO, OptionAction(opt_frame_size), "set frame size (WxH or abbreviation)", "size"},
    {"fs", OPT_BOOL, OptionAction(&is_full_screen), "force full screen"},
    {"an", OPT_BOOL, OptionAction(&audio_disable), "disable audio"},
    {"vn", OPT_BOOL, OptionAction(&video_disable), "disable video"},
    {"sn", OPT_BOOL, OptionAction(&subtitle_disable), "disable subtitling"},
    {"ast", OPT_STRING | HAS_ARG | OPT_EXPERT, OptionAction(&wanted_stream_spec[AVMEDIA_TYPE_AUDIO]), "select desired audio stream", "stream_specifier"},
    {"vst", OPT_STRING | HAS_ARG | OPT_EXPERT, OptionAction(&wanted_stream_spec[AVMEDIA_TYPE_VIDEO]), "select desired video stream", "stream_specifier"},
    {"sst", OPT_STRING | HAS_ARG | OPT_EXPERT, OptionAction(&wanted_stream_spec[AVMEDIA_TYPE_SUBTITLE]), "select desired subtitle stream", "stream_specifier"},
    {"ss", HAS_ARG, OptionAction(opt_seek), "seek to a given position in seconds", "pos"},
    {"t", HAS_ARG, OptionAction(opt_duration), "play  \"duration\" seconds of audio/video", "duration"},
    {"bytes", OPT_INT | HAS_ARG, OptionAction(&seek_by_bytes), "seek by bytes 0=off 1=on -1=auto", "val"},
    {"nodisp", OPT_BOOL, OptionAction(&display_disable), "disable graphical display"},
    {"f", HAS_ARG, OptionAction(opt_format), "force format", "fmt"},
    {"pix_fmt", HAS_ARG | OPT_EXPERT | OPT_VIDEO, OptionAction(opt_frame_pix_fmt), "set pixel format", "format"},
    {"stats", OPT_BOOL | OPT_EXPERT, OptionAction(&show_status), "show status", ""},
    {"fast", OPT_BOOL | OPT_EXPERT, OptionAction(&fast), "non spec compliant optimizations", ""},
    {"genpts", OPT_BOOL | OPT_EXPERT, OptionAction(&genpts), "generate pts", ""},
    {"drp", OPT_INT | HAS_ARG | OPT_EXPERT, OptionAction(&decoder_reorder_pts), "let decoder reorder pts 0=off 1=on -1=auto", ""},
    {"lowres", OPT_INT | HAS_ARG | OPT_EXPERT, OptionAction(&lowres), "", ""},
    {"sync", HAS_ARG | OPT_EXPERT, OptionAction(opt_sync), "set audio-video sync. type (type=audio/video/ext)", "type"},
    {"autoexit", OPT_BOOL | OPT_EXPERT, OptionAction(&autoexit), "exit at the end", ""},
    {"exitonkeydown", OPT_BOOL | OPT_EXPERT, OptionAction(&exit_on_keydown), "exit on key down", ""},
    {"exitonmousedown", OPT_BOOL | OPT_EXPERT, OptionAction(&exit_on_mousedown), "exit on mouse down", ""},
    {"loop", OPT_INT | HAS_ARG | OPT_EXPERT, OptionAction(&loop), "set number of times the playback shall be looped", "loop count"},
    {"framedrop", OPT_BOOL | OPT_EXPERT, OptionAction(&framedrop), "drop frames when cpu is too slow", ""},
    {"infbuf", OPT_BOOL | OPT_EXPERT, OptionAction(&infinite_buffer), "don't limit the input buffer size (useful with realtime streams)", ""},
    {"window_title", OPT_STRING | HAS_ARG, OptionAction(&window_title), "set window title", "window title"},
    // #if CONFIG_AVFILTER
    {"vf", OPT_EXPERT | HAS_ARG, OptionAction(opt_add_vfilter), "set video filters", "filter_graph"},
    {"af", OPT_STRING | HAS_ARG, OptionAction(&afilters), "set audio filters", "filter_graph"},
    // #endif
    {"rdftspeed", OPT_INT | HAS_ARG | OPT_AUDIO | OPT_EXPERT, OptionAction(&rdftspeed), "rdft speed", "msecs"},
    {"showmode", HAS_ARG, OptionAction(opt_show_mode), "select show mode (0 = video, 1 = waves, 2 = RDFT)", "mode"},
    {"default", HAS_ARG | OPT_AUDIO | OPT_VIDEO | OPT_EXPERT, OptionAction(opt_default), "generic catch all option", ""},
    {"i", OPT_BOOL, OptionAction(&dummy), "read specified file", "input_file"},
    {"codec", HAS_ARG, OptionAction(opt_codec), "force decoder", "decoder_name"},
    {"acodec", HAS_ARG | OPT_STRING | OPT_EXPERT, OptionAction(&audio_codec_name), "force audio decoder", "decoder_name"},
    {"scodec", HAS_ARG | OPT_STRING | OPT_EXPERT, OptionAction(&subtitle_codec_name), "force subtitle decoder", "decoder_name"},
    {"vcodec", HAS_ARG | OPT_STRING | OPT_EXPERT, OptionAction(&video_codec_name), "force video decoder", "decoder_name"},
    {"autorotate", OPT_BOOL, OptionAction(&autorotate), "automatically rotate video", ""},
    {
        NULL,
    },
};
...
int main(int argc, char **argv)
{
   ...
   flags = 0;//SDL_INIT_VIDEO | SDL_INIT_AUDIO | SDL_INIT_TIMER;
   if (audio_disable)
      flags &= 0;//~SDL_INIT_AUDIO;
   else
   {
