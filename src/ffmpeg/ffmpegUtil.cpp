...
AVCodec *find_encoder(const std::string &name, const AVMediaType type)
{
  ...
  if (!(codec = avcodec_find_encoder_by_name(name.c_str())) &&
      (desc = avcodec_descriptor_get_by_name(name.c_str())))
    codec = avcodec_find_encoder(desc->id);

  if (!codec) throw Exception("Unknown encoder " + name + "'");
...
AVCodec *find_decoder(const std::string &name, const AVMediaType type)
{
  ...
  if (!(codec = avcodec_find_decoder_by_name(name.c_str())) &&
      (desc = avcodec_descriptor_get_by_name(name.c_str())))
    codec = avcodec_find_decoder(desc->id);

  if (!codec) throw Exception("Unknown decoder " + name + "'");
...
{
  while ((AVDictionaryEntry *t = av_dict_get(
              b, "", t, AV_DICT_IGNORE_SUFFIX))) // for each entry in b
  {
    av_dict_set(&a, t->key, NULL,
                AV_DICT_MATCH_CASE); // if matching entry found, remove it
  }
}
...
void assert_avoptions(AVDictionary *m)
{
  if ((AVDictionaryEntry *t = av_dict_get(m, "", NULL, AV_DICT_IGNORE_SUFFIX)))
    throw Exception("Option %s not found. ", t->key);
}
...
//       return;
//    }
//    if (filename.empty() && (e = av_dict_get(st->metadata, "filename", NULL,
//    0)))
//       filename = e->value;
//    if (!*filename)
//    {
//       std::ostringstream msg;
//       msg << "No filename specified and no 'filename' tag in stream #" <<
//       st->index <<"."; throw Exception(msg.str());
//    }

//    assert_file_overwrite(filename);

//    if ((ret = avio_open2(&out, filename.c_str(), AVIO_FLAG_WRITE, &int_cb,
//    NULL)) < 0)
...
//       avio_check(filename.c_str(), 0) == 0)
//       {
//          if (stdin_interaction && !no_file_overwrite)
//          {
//             fprintf(stderr, "File '%s' already exists. Overwrite ? [y/N] ",
//             filename); fflush(stderr); term_exit(); signal(SIGINT, SIG_DFL);
//             if (!read_yesno())
//             {
