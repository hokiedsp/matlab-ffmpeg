...
///////////////////////////////////////////////////////////
SourceBase::SourceBase(Graph &fg, IAVFrameSourceBuffer &srcbuf)
    : EndpointBase(fg), buf(&srcbuf), eof(false)
{
}
...
{
  if (!issrc || pad > 0)
    throw Exception("[SourceBase::link] Source filter does not have an input "
                    "pad and has only 1 output pad.");
...
int SourceBase::processFrame()
{
  ...
  AVFrame *frame = buf->peekToPop(); // blocks till new frame is in then grab
                                     // the next frame to output (null if eof)
  int ret =
      av_buffersrc_add_frame_flags(context, frame, AV_BUFFERSRC_FLAG_KEEP_REF);
  if (!frame) eof = true;
...
AVFilterContext *VideoSource::configure(const std::string &name)
{
  ...
  // if (!par)
  //   throw Exception("[ffmpeg::filter::VideoSource::configure] Failed during
  //   av_buffersrc_parameters_alloc() call.");
...
//   sws_flags = SWS_BILINEAR + (ist->getCodecFlags(AV_CODEC_FLAG_BITEXACT) ?
//   SWS_BITEXACT : 0);
// }
...
std::string VideoSource::generate_args()
{
  ...
  const VideoParams &p =
      static_cast<const VideoParams &>(buf->getMediaParams());
...
bool VideoSource::updateMediaParameters()
{
  ...
  // check for validity
  if (p.format == AV_PIX_FMT_NONE || p.time_base.num <= 0 ||
      p.time_base.den <= 0 || p.width <= 0 || p.height <= 0 ||
      p.sample_aspect_ratio.num <= 0 || p.sample_aspect_ratio.den <= 0)
    return false;

  // if filter context has already been set, propagate the parameters to it as
  // well
  std::unique_ptr<AVBufferSrcParameters, decltype(av_free) *> par(
      av_buffersrc_parameters_alloc(), av_free);
  if (!par)
    throw Exception("[ffmpeg::filter::VideoSource::updateMediaParameters] "
  ...
  if (av_buffersrc_parameters_set(context, par.get()) < 0)
    throw Exception("[ffmpeg::filter::VideoSource::updateMediaParameters] "
                    "AVFilterContext could not accept parameters.");
...
bool AudioSource::updateMediaParameters()
{
  ...
  if (p.format <= AV_SAMPLE_FMT_NONE || p.time_base.num <= 0 ||
      p.time_base.den <= 0 || p.sample_rate <= 0 || p.channel_layout <= 0)
    return false;
  ...
  if (context)
  {
    std::unique_ptr<AVBufferSrcParameters, decltype(av_free) *> par(
        av_buffersrc_parameters_alloc(), av_free);
    if (!par)
      throw Exception("[ffmpeg::filter::AudioSource::updateMediaParameters] "
                      "Could not allocate AVBufferSrcParameters.");
