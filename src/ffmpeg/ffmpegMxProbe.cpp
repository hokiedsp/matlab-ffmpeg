...
void FFmpegMxProbe::open(const char *infile, AVInputFormat *iformat, AVDictionary *opts)
{
    int err, i;
    ...
    for (i = 0; i < (int)fmt_ctx->nb_streams; i++)
        st_dec_ctx.push_back(NewAVCodecContextUniquePtr(open_stream(fmt_ctx->streams[i], opts)));
...
int FFmpegMxProbe::getStreamIndex(const enum AVMediaType type, int wanted_stream_index) const
{
    if (!fmt_ctx)
        AVException::log(AV_LOG_FATAL, "No file is open.\n");
...
double FFmpegMxProbe::getVideoFrameRate(int wanted_stream_index, const bool get_avg) const
{
...
double FFmpegMxProbe::getVideoFrameRate(const std::string &spec_str, const bool get_avg) const
{
...
#define mxSetRatioField(fname, fval)            \
    ...
    if (fmt_ctx->nb_programs)
    {
        ...
        for (j = 0; j < (int)fmt_ctx->nb_programs; j++)
        {
            ...
            mxArray *mxStreams = createMxStreamStruct(fmt_ctx->programs[j]->nb_stream_indexes);
            for (k = 0; k < (int)fmt_ctx->programs[j]->nb_stream_indexes; k++)
            {
                dump_stream_to_matlab(fmt_ctx->programs[j]->stream_index[k], mxStreams, k);
    ...
    for (int i = 0; i < (int)fmt_ctx->nb_streams; i++)
        if (notshown[i])
        {
            dump_stream_to_matlab(i, mxStreams, j++);
        }
}

void FFmpegMxProbe::dump_stream_to_matlab(const int sid, mxArray *mxInfo, const int index) const
{
...
{
    return mxCreateStructMatrix(size, 1, ARRAY_LENGTH(stream_field_names), stream_field_names);
}
...
const char *FFmpegMxProbe::chapter_field_names[] = {
    "start",
    "end",
    "metadata"};

const char *FFmpegMxProbe::program_field_names[] = {
    "id",
    "name",
    "metadata",
    "streams"};

const char *FFmpegMxProbe::stream_field_names[] = {
    "index",
    "codec_name",
    "codec_long_name",
    "profile",
