...
const AVPixFmtDescriptor &MediaReader::getPixFmtDescriptor() const
{
...
void MediaReader::open_input_file(const std::string &filename)
{
  int ret;
  AVCodec *dec;
...
void MediaReader::close_input_file()
{
...
void MediaReader::destroy_filters()
{
  if (filter_graph)
    avfilter_graph_free(&filter_graph);
}

void MediaReader::create_filters(const std::string &filter_description, const AVPixelFormat pix_fmt_rq)
{
...
void MediaReader::setCurrentTimeStamp(const double val, const bool exact_search)
{
  if (!isFileOpen())
...
void MediaReader::setFilterGraph(const std::string &filter_desc, const AVPixelFormat pix_fmt) // stops
{
  if (!isFileOpen())
    throw ffmpegException("No file open.");
...
void MediaReader::start()
{
  ...
  packet_reader = std::thread(&MediaReader::read_packets, this);
  frame_filter = std::thread(&MediaReader::filter_frames, this);
...
{
  // lock all mutexes
  // av_log(dec_ctx, AV_LOG_INFO, "ffmpeg::MediaReader::pause()::start\n");
  ...
  // av_log(dec_ctx, AV_LOG_INFO, "ffmpeg::MediaReader::pause()::paused\n");
}
...
void MediaReader::read_packets()
{
  ...
  try
  {
    ...
    while (!killnow)
    {
      ...
      {
        // av_log(NULL,AV_LOG_INFO,"ffmpeg::MediaReader::read_packets()::turned off\n");
        std::unique_lock<std::mutex> reader_guard(reader_lock);
        reader_status = IDLE;
        ...
        if (killnow)
          break;
        last_frame = false;
        // av_log(NULL,AV_LOG_INFO,"ffmpeg::MediaReader::read_packets()::turned on\n");
      }
      ...
      if (!last_frame)
      {
        ...
        // av_log(NULL, AV_LOG_INFO, "ffmpeg::MediaReader::read_packets()::read t=%d [%d]\n", packet.pts, ret);
      }
      ...
      if (last_frame || reader_status == PAUSE_RQ)
      {
        ...
        // av_log(NULL, AV_LOG_INFO, "ffmpeg::MediaReader::read_packets()::EOF sent to decoder\n", packet.pts, ret);
      }
      ...
      if (last_frame)
      {
        ...
        // av_log(NULL,AV_LOG_INFO,"ffmpeg::MediaReader::read_packets()::reader_ready notified after last frame\n");
      }
    }
  }
  catch (...)
  ...
  // av_log(NULL,AV_LOG_INFO,"ffmpeg::MediaReader::read_packets()::exiting\n");
  av_packet_unref(&packet);
}

void MediaReader::filter_frames()
{
  int ret;
  ...
  try
  {
    ...
    while (!killnow)
    {
      ...
      // av_log(NULL, AV_LOG_INFO, "ffmpeg::MediaReader::filter_frames()::new decoded frame t=%d\n", frame->pts);
      ...
      if (last_frame)
      {
        ...
        if (pause_rq)
        {
          // av_log(NULL,AV_LOG_INFO,"ffmpeg::MediaReader::filter_frames()::notifying buffer_flushed\n");
  ...
  {
    av_log(NULL, AV_LOG_FATAL, "ffmpeg::MediaReader::filter_frames() thread threw exception.\n");
  ...
  // av_log(NULL, AV_LOG_FATAL, "ffmpeg::MediaReader::filter_frames()::exiting.\n");
}

void MediaReader::copy_frame_ts(const AVFrame *frame)
{
  if (frame)
  {
    if (!firstframe)
    ...
    if (buf_start_ts)
    {
      if (frame->best_effort_timestamp < buf_start_ts)
        // {  av_log(NULL,AV_LOG_INFO,"ffmpeg::MediaReader::copy_frame_ts::dropping t=%d < %d\n",frame->best_effort_timestamp,buf_start_ts);
        return;
      else
        buf_start_ts = 0;
    }
  ...
  // if (!flush_frames)
  //   av_log(NULL, AV_LOG_INFO, "ffmpeg::MediaReader::copy_frame_ts::buffering t=%d\n", frame ? frame->pts : -1);
...
void MediaReader::resetBuffer(FrameBuffer *new_buf)
{
  std::unique_lock<std::mutex> buffer_guard(buffer_lock);
  buf = new_buf;
  buffer_ready.notify_one();
...
FrameBuffer *MediaReader::releaseBuffer()
{
...
size_t MediaReader::blockTillBufferFull()
{
  if (!isFileOpen() || !buf)
    return 0;
  std::unique_lock<std::mutex> buffer_guard(buffer_lock);
...
size_t MediaReader::blockTillFrameAvail(size_t min_cnt)
{
  if (!isFileOpen() || !buf)
    return 0;
  std::unique_lock<std::mutex> buffer_guard(buffer_lock);
